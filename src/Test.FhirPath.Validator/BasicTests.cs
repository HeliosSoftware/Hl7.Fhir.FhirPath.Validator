...
namespace Test.Fhir.FhirPath.Validator
{
    ...
    public class BasicTests
    {
        ...
        public void TestMethodVariableUsingNoPrefix()
        {
            ...
            Console.WriteLine(visitor.Outcome.ToXml(new FhirXmlSerializationSettings() { Pretty = true }));
            Assert.IsFalse(visitor.Outcome.Success);
            Assert.AreEqual(2, visitor.Outcome.Issue.Count);
            Assert.AreEqual("prop 'surprise' not found on Questionnaire (did you mean to use the variable '%surprise' : HumanName)", visitor.Outcome.Issue[0].Details.Text);
        }
